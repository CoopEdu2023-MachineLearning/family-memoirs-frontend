import { AutoComplete, Flex, List, Space, Tag, Typography } from "antd";
import { DeleteOutlined } from "@ant-design/icons";
import { SearchBox } from "@pages/SearchResultsPage";
...
const SuggestionPanel = ({ stories, tellers, inputValue, onSelect, onStorySelect }) => {
    ...
    return (
        <Space direction="vertical" style={{ padding: 8, width: "100%", boxSizing: "border-box" }}>
            {inputValue ? (
                <>
                    <ListOptionGroup
                        title="内容"
...
export const AutoCompleteSearch = ({ stories, tellers, refine }) => {
    ...
    return (
        <AutoComplete
            ...
            dropdownRender={() => <SuggestionPanel stories={stories} tellers={tellers} inputValue={inputValue} onSelect={handleSearch} onStorySelect={(id) => {
                navigate(`/article/${id}`);
            }} />}
            ...
            value={inputValue}
        >
            <SearchBox refine={refine} defaultValue={""} />
        </AutoComplete>
    );
};